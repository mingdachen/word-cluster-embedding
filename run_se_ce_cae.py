...
def get_args():
    ...
    parser.add_argument('--aux_dim', type=int, default=0,
                        help='dimension of auxiliary vector for \
                        each word (default: 0)')
    ...
    parser.add_argument('--proj', type=str, default="standard",
                        help='types of embedding projection: standard (default), \
                        gumbel')
    ...
    parser.add_argument('--init_temp', type=float, default=0.9,
                        ...
                        (default: 0.9)')
    parser.add_argument('--anneal_rate', type=float, default=0.0,
                        help='annealing rate for temperature (default: 0.0)')
...
def run(args):
    ...
    if not os.path.isfile(args.vocab_file):
        vocab = data_utils.make_dict(
            vocabs.get("train"), args.vocab_file,
            args.max_words, args.save_vocab)
    else:
        vocab = data_utils.load_dict(args.vocab_file)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(args.n_epoch):
            ...
            logging.info("test acc: {:.5f}, #pred: {}, elapsed time: {:.1f}(s)"
...
if __name__ == '__main__':
    ...
    args.save_dir = "experiments" + "/" + args.dataset.lower() + "/" \
        + args.opt.lower() + "_ce" + str(args.proj) \
